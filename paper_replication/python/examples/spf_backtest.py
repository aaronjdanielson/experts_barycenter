...
def run_backtest(horizon: str = "current"):
    ...
    try:
        ...
        # Energy (CPILFESL), December-over-December percent change.
        # 2025: BLS release 2026-01-13 (12-month change Dec 2025 = 2.6%).
        hardcoded = {
...
def _plot_reliability(df: pd.DataFrame, series: str):
    ...
    fig.savefig(fname, bbox_inches="tight")
    fig.savefig(FIG_DIR / f"fig_bt8_reliability_{series}.pdf", bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved {fname.name}")
